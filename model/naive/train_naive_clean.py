...
# -*- coding: utf-8 -*-
'''
Baseline limpio (sin fuga): AutoML (naiveautoml) 1X2 con split temporal H1->H2.

Usa las MISMAS features 100% pre‑partido que el baseline limpio:
...
Se entrena SOLO con la primera mitad de la temporada y evalúa en la segunda.
Simula banca con Kelly fraccional y umbral EV exactamente igual al baseline.

Ejemplo:
  python train_naive_clean.py \
    --csv ../../data/inputs/premier_dataset_final.csv \
    --season latest \
    --ev_threshold 1.02 \
    --kelly_frac 0.25 \
...
# Intentamos importar naiveautoml
try:
    import naiveautoml
except Exception as e:
    print("ERROR: No se pudo importar 'naiveautoml'.", file=sys.stderr)
    print("Instálalo con: pip install naiveautoml", file=sys.stderr)
    raise
...
def kelly_fraction(p: float, o: float, frac: float = 0.25) -> float:
    raw = (o*p - 1) / (o - 1) if o and (o-1) else 0.0
...
def main():
    ...
    # Opcionales para naiveautoml
    ap.add_argument("--time_limit", type=int, default=60, help="Tiempo (s) para búsqueda de modelos en naiveautoml")
    ap.add_argument("--seed", type=int, default=42, help="Semilla para reproducibilidad")
    args = ap.parse_args()

    df = pd.read_csv(args.csv, parse_dates=["date"])

    needed = {"season","result","odd_home","odd_draw","odd_away","is_first_half","date","home_team","away_team"}
    missing = [c for c in needed if c not in df.columns]
    if missing:
        raise SystemExit(f"Missing required columns: {missing}")

    # Filtramos columnas seguras
    ...
    # Imputación simple por mediana (coherente con baseline)
    ...
    # Entrenar
    naml.fit(trainX, trainY, categorical_features=[])  # features ya son numéricas
    ...
    if hasattr(model, "predict_proba"):
        ...
        # Si el modelo no soporta predict_proba, creamos una distribución casi-degenerada
        preds_tmp = model.predict(testX)
        proba = np.zeros((len(preds_tmp), 3), dtype=float)
        for i, p in enumerate(preds_tmp):
            proba[i, int(p)] = 1.0
    ...
    # Ensamblar dataframe de salida con probabilidades y predicción
    pred_df = test.copy()
    pred_df[["p_H","p_D","p_A"]] = proba
    pred_df["pred"] = [REV_TARGET_MAP[int(i)] for i in preds]
    ...
    preds_path = outdir / f"predictions_naive_{season_safe}.csv"
    hist_path  = outdir / f"bankroll_naive_{season_safe}.csv"
    meta_path  = outdir / f"metrics_naive_{season_safe}.txt"
    ...
    with open(meta_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"Chosen model: {type(model).__name__}\n")
    ...
    print("=== AutoML (naiveautoml) CLEAN (Multiclass 1X2) ===")
    print(f"Season: {season}")
    print(f"SAFE features used ({len(feat_cols)}): {feat_cols}")
    print(f"Chosen model: {type(model).__name__}")
